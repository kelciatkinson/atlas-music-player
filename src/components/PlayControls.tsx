...
export default function PlayControls({onPrev, onNext, canGoPrev, canGoNext,}: PlayControlProps) {
  ...
  const onShuffle = () => setIsShuffed(!isShuffled);
  ...
  return (
    <div id="play-controls" className="w-100 flex justify-between -mt-64">
      ...
      <button id="play" onClick={onPause} className="group transition duration-300 ease-in-out w-12 h-12 flex border-(--color-text) hover:border-(--color-dark-text) dark:border-(--color-dark-text) justify-center border-solid border-2 rounded-md items-center self-center">
        {isPlaying ? (
